...
class multi_player():
    ...
    def show(self,window:pygame.Surface):
        ...
        step_text=Text((window_width*3//5,window_height*3//18),step_str)
        # chess=Chessboard((window_width//2,window_height//2),(window_width,window_height))
        chess=Chessboard((window_width//5,window_height//3),(window_width*9//25,window_height*17//27),len(self.data),background_color=(181,170,156))
        chess.update(self.data)
        ##道具还没写
        exit_str='admit defeat'
        exit_button=Button((window_width*3//5,window_height*5//18),exit_str,size=(250,50))
        show_list=Need_to_show([score_text,step_text,chess,exit_button])
        
        another_score_str='score'+str(self.another_score)
        another_score_text=Text((window_width*3//8,window_height*11//16),another_score_str)
        another_step_str='step'+str(self.another_step)
        another_step_text=Text((window_width*3//8,window_height*13//16),another_step_str)
        # chess=Chessboard((window_width//2,window_height//2),(window_width,window_height))
        another_chess=Chessboard((window_width*4//5,window_height*2//3),(window_width*9//25,window_height*17//27),len(self.another_data),background_color=(181,170,156))
        another_chess.update(self.another_data)
        ##道具还没写
        another_exit_button=Button((window_width*2//3,window_height*15//16),exit_str,size=(250,50),background_color=(0,0,0,100))
        show_list=Need_to_show([score_text,step_text,chess,exit_button,another_score_text,another_step_text,another_chess,another_exit_button])
        show_list.update(window,bg_img=self.bg_img)
        
    def update(self,data,score,step,another_data,another_score,another_step,item_list=[],item_pos=[],another_item_list=[],another_item_pos=[]):
        self.data=data
        self.score=score
        self.step=step
