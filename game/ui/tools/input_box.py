...
class InputBox(pygame.sprite.Sprite):
    ...
    def show(self, window: pygame.Surface):
        ...
        if self.text != '':
            ...
            self.underline = '_'
        text_show = (
            text_font.render(self.text + self.underline, True, self.click_color)
            if self.ready
            else text_font.render(self.text + self.underline, True, self.color)
        )
    ...
    def onclick(self, mouse_pos):
        ...
        rect.center = self.center
        if (
            mouse_pos[0] > rect.x - 30
            and mouse_pos[0] < rect.x + 30 + rect.width
            and mouse_pos[1] > rect.y - 30
            and mouse_pos[1] < rect.y + rect.height + 30
        ):
            self.ready = True
            return True
        else:
